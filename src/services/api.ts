...
export const nucleusApi = {
  ...
  getOverview: (connectionId: string) => api.get(`/nucleus/${connectionId}`),
  ...
  submitAnswer: (connectionId: string, questionId: string, response: any) =>
    api.post(`/nucleus/${connectionId}/questions/${questionId}/answer`, {
      response,
    }),
  ...
  // Voice section
  getVoice: (connectionId: string) => api.get(`/nucleus/${connectionId}/voice`),
  ...
  // Mini games
  getGames: (connectionId: string) => api.get(`/nucleus/${connectionId}/games`),
  ...
  submit2Truths1Lie: (connectionId: string, gameId: string, statements: any) =>
    api.post(
      `/nucleus/${connectionId}/games/${gameId}/truth-or-lie`,
      statements,
    ),

  guessTheLie: (connectionId: string, gameId: string, guessIndex: number) =>
    api.post(`/nucleus/${connectionId}/games/${gameId}/guess-lie`, {
      guessIndex,
    }),
};
...
export const missionApi = {
  getCurrentRound: (connectionId: string) =>
    nucleusApi.getOverview(connectionId),
  getCurrent: (connectionId: string) => nucleusApi.getOverview(connectionId),
  vote: (_connectionId: string, _missionOptionId: string) =>
...
export const sparksApi = {
  ...
  purchasePack: (packId: string) => api.post("/sparks/purchase", { packId }),
  ...
  canAfford: (action: string) => api.get(`/sparks/can-afford?action=${action}`),
